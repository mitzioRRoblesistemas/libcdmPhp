...
class CDMValidacionFacial
{
    ...
    {            
        ...
        $solicitud = $request->cookie('cdm-solicitud-facetec');
        ...
        try {
            if ($solicitud) {
                ...
                if ($rta['status'] == 200) {
                    if ($rta['data']['resultadoProceso'] !== "correcto") {
                        Cookie::queue('cdm-solicitud-facetec', '', -1);
            ...
            $rta = $instanciaSDK->api_validaFaceTec($request->cookie('cdm-token'), $request->fullUrl());
